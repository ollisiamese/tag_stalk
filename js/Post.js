...
if(self.type=="audio"){
 ...
 self.audioplayer=data.embed;
  if(data.caption){
  self.caption(data.caption); //self.caption is a ko.observable, unlike the rest of the Post's properties, because it is used with an "if" data binding in the html view (either the caption is shown, or the title+artist for the audio file, but not both)
  }
  else {
  self.caption("");
   }
...
if(self.type=="text"){
 ...
 if(data.body!=""){self.body=data.body;}
 else{self.body=null;}
}
...
self.answer=data.aswer;
	...
	somenamespace.Post=Post; //get public access
